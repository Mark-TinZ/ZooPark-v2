...
const Img_scene* MainWindow::findImageByName(const std::vector<Img_scene>& imageList, const std::string& name) {
...
void MainWindow::Game() {
	...
	if (ImGui::BeginTabItem("Животные")) {
		...
		const Img_scene* marketImg = findImageByName(images, "animal");
		if (marketImg && marketImg->textureID != 0 && !show_advice_animal) {
			...
			if (ImGui::BeginPopupModal("Санбой", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove)) {
				...
				ImGui::Image((ImTextureID)(uintptr_t)marketImg->textureID, ImVec2(250, 250));
				ImGui::SameLine();
				ImGui::TextWrapped(
	...
	if (ImGui::BeginTabItem("Вольеры")) {
		...
		// Изображение вольера
		const Img_scene* enclosureImg = findImageByName(images, "enclosure");
		...
		if (enclosureImg && enclosureImg->textureID != 0 && !show_advice_enclosure) {
			...
			if (ImGui::BeginPopupModal("Санбой", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove)) {
				ImGui::SetWindowSize(ImVec2(900, 330)); // Устанавливаем фиксированный размер окна
				ImGui::Image((ImTextureID)(uintptr_t)enclosureImg->textureID, ImVec2(250, 250));
				ImGui::SameLine();
				ImGui::TextWrapped(
	...
	if (ImGui::BeginTabItem("Персонал")) {
		...
		const Img_scene* shopImg = findImageByName(images, "worker");
		if (shopImg && shopImg->textureID != 0 && !show_advice_worker) {
			ImGui::OpenPopup("Санбой");

			if (ImGui::BeginPopupModal("Санбой", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove)) {
				ImGui::SetWindowSize(ImVec2(900, 330)); // Устанавливаем фиксированный размер окна
				ImGui::Image((ImTextureID)(uintptr_t)shopImg->textureID, ImVec2(250, 250));
				ImGui::SameLine();
				ImGui::TextWrapped(
	...
	if (ImGui::BeginTabItem("Магазин")) {
		...
		// Изображение магазина
		const Img_scene* shopImg = findImageByName(images, "shop");
	...
	if (ImGui::BeginTabItem("Статистика")) {
		...
		*/
		...
		// Изображение магазина
		const Img_scene* shopImg = findImageByName(images, "statistic");
	...
	if (ImGui::BeginTabItem("Рынок")) {
		...
		const Img_scene* image = findImageByName(images, "market");
		for (int y = 0; y < 4; y++)
				for (int x = 0; x < 4; x++)
				{
					if (x > 0)
